...
2. `x-apptainer` values are passed through verbatim into the command right after the `run`/`exec` verb. If the flag is a bare word needing a `--` prefix (e.g. `writable-tmpfs` → `--writable-tmpfs`), add it to the allowlist in `state_x_apptainer`; flags already in full form (e.g. `--foo`) need no code change.
...
- 56 subTests total (13 compose yaml parsing + 13 compose yaml execution + 29 docker compose cli execution + 1 apptainer cli parsing). As of the 2026-09-02 run the **apptainer side is fully green**; 17 subtests error on the docker side only, due to daemon issues (see Known quirks: address pool exhaustion + libnetwork desync) — re-run when the daemon is healthy. Implemented compose yaml cases all green: `command`, `environment` (dict **and** list form), `volumes`, `hostname` (`--hostname`), `working_dir` (`--cwd`), `dns` (`--dns`, comma-joined), `security_opt` (`no-new-privileges` → `--security no_new_privs`), `entrypoint` (service `entrypoint` → verb switches `run` → `exec`, entrypoint list prepended before the command), `labels` (accepted and ignored — apptainer has no runtime label flag), `read_only` (`false` → `--writable-tmpfs`; `true` → no flag, apptainer default), `init` (`false` → `--no-init`; `true` → no flag, apptainer default), top-level `version` (accepted and ignored — it broke the parser before). Docker compose cli cases green: `up`, `-f <file>`, `up <service>`, plus 26 new flag cases (all accepted and ignored at runtime — apptainer has no equivalent, so they are dropped; pure `parse_args` additions): global `-p/--project-name`, `--project-directory`, `--ansi`, `--parallel`, `--compatibility`, `--all-resources`; pull/build `--pull`, `--quiet-pull`, `--build`, `--no-build`, `--quiet-build`; exit/attach `--abort-on-container-exit`, `--abort-on-container-failure`, `--exit-code-from`, `--no-color`; lifecycle `--no-deps`, `--no-recreate`, `--force-recreate`, `--always-recreate-deps`, `--remove-orphans`, `-V/--renew-anon-volumes`, `-t/--timeout`, `-y/--yes`, `--attach`, `--no-attach`, `--scale`. (`parse_args` now supports `-f/--file`, `-v/--verbose`, `--dry-run` + all 26 flags above; subcommands `up` [optional service names], `build`, `run`. The `-p` case is still `open` — its docker side hit the libnetwork desync in the 2026-09-02 run.) Apptainer cli case green: `--writable-tmpfs` (the `x-apptainer` compose key — a list of raw apptainer flags inserted after the `run`/`exec` verb; only `writable-tmpfs` gets a `--` prefix, other values pass through verbatim).
...
- **docker daemon libnetwork desync (2026-09-02, unfixed):** after heavy network create/delete churn (e.g. probing many `docker-compose` projects + `docker network prune` in one session), dockerd's in-memory libnetwork state desyncs from its DB: `docker-compose up` fails with `failed to set up container networking: network <id> not found`, where `<id>` is a stale ID of an already-removed network (no client-side state file holds it — it is in daemon memory; intermittent, some runs still succeed). Fix = dockerd restart, deferred per Ground rules (do not fix during a session — remark and continue). Until then: docker-compose-side checks in `test_2`/`test_3` may fail for reasons unrelated to the change under test; judge the apptainer side first and re-run the suite when the daemon is healthy again. Avoid `docker network prune` during a session.
- **docker daemon address pool exhaustion (2026-09-02, unfixed):** once enough `docker-compose` projects have each created their `<project>_default` bridge network in one session, the daemon's predefined address pools are fully subnetted and network *creation* fails: `docker-compose up` errors with `failed to create network <project>_default: Error response from daemon: all predefined address pools have been fully subnetted`. Distinct from the libnetwork desync above (this fails at create time, not at container-networking setup). Fix = free a subnet by removing unused project networks, or a dockerd restart; deferred per Ground rules (do not fix during a session — remark and continue). Until then the docker-side checks in `test_2`/`test_3` may fail on this; judge the apptainer side first.
