...
- 20/20 subTests pass (unittest reports `Ran 2 tests ... OK`). Implemented cases all green: `command`, `environment`, `volumes`, `hostname` (`--hostname`), `working_dir` (`--cwd`), `dns` (`--dns`, comma-joined), `security_opt` (`no-new-privileges` → `--security no_new_privs`), `entrypoint` (service `entrypoint` → verb switches `run` → `exec`, entrypoint list prepended before the command), `labels` (accepted and ignored — apptainer has no runtime label flag).
