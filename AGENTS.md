...
- `main()`: `up` executes **all** services sequentially (services with a non-empty `profiles` list are skipped — compose profiles have no active-profile selection yet; an explicit `run <service>` ignores profiles, matching docker); `run <service> [cmd...]` executes only that service, with `cmd` overriding its `command` (use `--` before commands that start with `-`), unknown service → error exit 1.
